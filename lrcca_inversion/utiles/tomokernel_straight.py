...
def tomokernel_straight_2D(data, x, z):
    ...
    if (
        xmin > np.min(np.vstack((data[:, 0], data[:, 2])))
        or xmax < np.max(np.vstack((data[:, 0], data[:, 2])))
        or zmin > np.min(np.vstack((data[:, 1], data[:, 3])))
        or zmax < np.max(np.vstack((data[:, 1], data[:, 3])))
    ):
    ...
    #    zmid = np.arange((zmin+x_values/2),(zmax-x_values/2),x_values)    # z-coordinates of cell midpoints
    ...
    maxelem = int(np.round(nrays * np.sqrt(nx**2 + nz**2)))
    ...
    for i in range(0, nrays):
        ...
        ip1 = np.append(
            xcellb, xs + (zcellb - zs) * 1 / (slope + 1e-20)
        )  # x-coords of all intersection points
        ip2 = np.append(
            zs + (xcellb - xs) * slope, zcellb
        )  # z-coords of all intersection points
        ...
        cmidpt = (
            0.5
            * np.vstack(
                (ipoint[0:-1, 0] + ipoint[1:, 0], ipoint[0:-1, 1] + ipoint[1:, 1])
            ).T
        )
